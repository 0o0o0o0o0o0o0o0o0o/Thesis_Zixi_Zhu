...
```{r}
#5 single, 239 binary, 176 category, 79 numeric
drop_new <- c()
bi <- c()
category <- c()
numeric <- c()
...
```{r}
cluster_assignment_sub <- cluster_assignment[cluster_assignment!=3]
```
...
setwd("D:\\Thesis\\WORKSPACE2024\\Hierarchical Clustering\\data")
write.table(new_case1,file = "data_no_multicol_allgender.txt",quote = F,row.names = T,sep="\t")
```
