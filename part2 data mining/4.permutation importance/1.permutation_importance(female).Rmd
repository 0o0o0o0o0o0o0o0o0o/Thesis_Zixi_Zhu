...
keep cluster 1 and 2

```{r}
round_case <- round_case[cluster_assignment==1|cluster_assignment==2,]
```

**see variable type**

```{r}
#14 single, 234 binary, 178 category, 73 numeric
drop_new <- c()
bi <- c()
category <- c()
numeric <- c()
...
```{r}
cluster_assignment_sub <- cluster_assignment[cluster_assignment==1|cluster_assignment==2]
```
...
setwd("D:\\Thesis\\WORKSPACE2024\\Hierarchical Clustering\\data")
write.table(new_case1,file = "data_no_multicol_female.txt",quote = F,row.names = T,sep="\t")
```
